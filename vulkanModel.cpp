...
namespace hva {
    ...
    VulkanModel::VulkanModel(VulkanDevice &device, const std::vector<Vertex> &vertices, const std::vector<uint32_t>& indices, VkQueue transferQueue, VkCommandPool transferCommandPool, int newTexID) : device{device}{
        ...
        modelTransferQueue = transferQueue;
        modelTransferCommandPool = transferCommandPool;
        texID = newTexID;
    }
    ...
    void VulkanModel::updateModel(const std::vector<Vertex> &vertices, const std::vector<uint32_t>& indices) {
        vkDestroyBuffer(device.device(), vertexBuffer, nullptr);
        vkDestroyBuffer(device.device(), indexBuffer, nullptr);
        vkFreeMemory(device.device(), vertexBufferMemory, nullptr);
        vkFreeMemory(device.device(), indexBufferMemory, nullptr);
        createVertexBuffers(vertices, modelTransferQueue, modelTransferCommandPool);
        createindexBuffers(indices, modelTransferQueue, modelTransferCommandPool);
    }
