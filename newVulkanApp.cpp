...
namespace hva{
    ...
    void NewVulkanApp::loadModels() {
        ...
        Plane p1;
        Point p(glm::vec2(0.0f));
        graph1.addChild(p);

        for(int i; i<MAX_OBJECTS; i++){
            graph.push_back(std::move(std::make_unique<VulkanModel>(device, graph1.getVert(), graph1.getInd() , device.graphicsQueue(), device.getCommandPool(), createTexture("",""))));
        }
    ...
    void NewVulkanApp::createPipelineLayout() {
        ...
        if (TEXTURE) {
            ...
            pipelineLayoutInfo.setLayoutCount = static_cast<uint32_t>(1);
    ...
    void NewVulkanApp::recordCommand(uint32_t imageIndex){
        ...
        for (int p=0; p<1; p++) {
            for (int m = 0; m < graph.size(); m++) {
                ...
                graph[m]->bind(commandBuffers[imageIndex]);
                graph[m]->bindIndexed(commandBuffers[imageIndex]);
                ...
                if (TEXTURE) {
                    std::array<VkDescriptorSet, 2> descriptorSetGroup = {descriptorSets[imageIndex],
                                                                            samplerDescriptorSets[graph[m]->getTexID()]};
                ...
                graph[m]->drawIndexed(commandBuffers[imageIndex]);
            }
        }
    ...
    void NewVulkanApp::updateModels() {
        ...
        float dir = M_PI * 2.0f * rand()/double(RAND_MAX);

        for(int i=0; i<graph.size(); i++){
            glm::mat4 prevM = graph[i]->getModel()->M;
            graph[i]->setModel(glm::translate(prevM , glm::vec3(increment * cos(dir),increment * sin(dir),0.0f)));
            dir = M_PI * 2.0f * rand()/double(RAND_MAX);
        }
    }
